...
Avoidance.RECOVERY_ADMISSION_INTERVAL = 5.00
-- 新基线先限制塔辅助占权；通用技能执行器将在感知证据通过后接入。
Avoidance.CONTROL_LEASE_TIME = 2.00
Avoidance.SOURCE_OBSERVATION_ENABLED = false
-- S3只接入已验证早苗来源；有限脱离与已知任务目标绕行。
Avoidance.SKILL_AVOIDANCE_ENABLED = true
Avoidance.SKILL_SCAN_INTERVAL = 0.25
Avoidance.SKILL_SCAN_RANGE = 1800
Avoidance.SKILL_SAFETY_MARGIN = 32
-- 主动脱离多走一段后交还；同来源进入边界独立保留至到期。
Avoidance.SKILL_EXIT_HYSTERESIS = 64
-- S5首轮权重：以生命比例和追击减速代价判定是否值得接管。
Avoidance.SKILL_ACCEPTABLE_RISK = 0.18
Avoidance.SKILL_RETREAT_BRIDGE_TIME = 3.0
Avoidance.SKILL_ESCAPE_TIME = 2.00
Avoidance.SKILL_STALL_TIME = 0.65
Avoidance.SKILL_LOOKAHEAD_TIME = 2.00
Avoidance.SKILL_TIME_MARGIN = 0.25
Avoidance.OBSERVATION_DIAGNOSTICS = true
Avoidance.RECOVERY_MAX_DISTANCE = 1400
-- 仅已校验的短步/细步绕过Bot寻路层；缺少API/动作枚举时保留普通移动。
Avoidance.RECOVERY_DIRECT_MOVE_ENABLED = false
...
-- 健康且塔伤可承受的高等级团战允许双 T4；三塔重叠仍规避。
Avoidance.HIGH_LEVEL_TEAMFIGHT_MAX_HIGH_GROUND_TOWERS = 2
Avoidance.HIGH_LEVEL_TEAMFIGHT_POLICY_LOG_INTERVAL = 1.00
Avoidance.MODE_HANDOFF_STALE_TIME = 1.00
Avoidance.HANDOFF_FALLBACK_ENABLED = false
