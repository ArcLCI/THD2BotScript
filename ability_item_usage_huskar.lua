...
function ConsiderAbilityMinoriko01()
	...
	--出灵异珠之后
		...
		for _,allyUnit in pairs(GetUnitList(UNIT_LIST_ALLIES))
		do
			if allyUnit:GetUnitName() == "npc_thdots_unit_minoriko02_box" then
				local tableNearbyEnemyHeroes = CachedGetNearbyHeroes( allyUnit, 500, true, BOT_MODE_NONE )
				if #tableNearbyEnemyHeroes > 2 then
					return BOT_ACTION_DESIRE_HIGH, allyUnit
				end
			end
		end
		...
		for  _,npcEnemy in pairs( tableNearbyEnemyHeroes )
		do
			if not IsPossibleIllusion( npcEnemy ) and GetUnitToLocationDistance(npcEnemy,locationAoE.targetloc) < minDist then
				minDist = GetUnitToLocationDistance(npcEnemy,locationAoE.targetloc)
				target = npcEnemy
			end
		end
