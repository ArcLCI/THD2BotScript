        ...
        then
            ...
            if J.IsInRange(bot, u, 1200) then
                ...
                if bot:GetTeam() ~= u:GetTeam() then
                    if string.find(name, 'npc_dota_phoenix_sun')
                    then
...
function X.ShouldRun()
	if bot:IsChanneling() or not bot:IsAlive() then return 0 end
    ...
    if enemyFountainDistance < 1000 then return 2 end
